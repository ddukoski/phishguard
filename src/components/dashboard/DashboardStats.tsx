...
export default function DashboardStats({ stats }: DashboardStatsProps) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <StatTile label="Completed Scenarios" value={stats.completed} />
      <StatTile label="Correct" value={stats.correct} />
      <StatTile label="Accuracy" value={`${stats.accuracy_rate}%`} />
      <StatTile label="Total Score" value={stats.total_score} />
      <StatTile label="Avg. Response Time" value={`${stats.average_time_seconds}s`} />
    </div>
  );
}
